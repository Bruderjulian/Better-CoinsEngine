...
/**
 * Redis synchronization manager for ExcellentEconomy
 * Provides cross-server currency balance synchronization, leaderboard sync, and
 * user data sync
 */
public class RedisSyncManager {
    ...
    private void syncLeaderboards() {
        if (!isActive() || !this.plugin.getTopManager().isPresent())
            return;
    ...
    private void startSubscriber() {
        ...
        }, "ExcellentEconomy-RedisSubscriber");
    ...
    private void applyLeaderboardUpdate(JsonObject data) {
        ...
        this.plugin.runNextTick(() -> {
            this.plugin.getTopManager().ifPresent(topManager -> {
                ...
                this.plugin
                        .info("Updated leaderboard for currency: " + currencyId + " (" + entries.size() + " entries)");
            });
        });
    }
    ...
    private void applyPaymentNotification(JsonObject data) {
        ...
        this.plugin.runNextTick(() -> {
            ...
            su.nightexpress.excellenteconomy.api.currency.Currency currency = this.plugin.getCurrencyRegistry()
                    .getById(currencyId);
            if (currency == null)
                return;
    ...
    private void handleUserCreateRequest(JsonObject data) {
        ...
        if (requestingNode.equals(this.nodeId))
            return;
